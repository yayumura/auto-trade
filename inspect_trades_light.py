...
def run_diagnostic():
    # 軽量版: 主要4銘柄のみ
    test_universe = ["8306", "7203", "7201", "6758"]
    tickers = [f"{code}.T" for code in test_universe] + ["1321.T"]
    print("Downloading LIGHT data...")
    ...
    print(f"Running LIGHT backtest from {timeline[0]} to {timeline[-1]}...")
    ...
    for t in trade_history:
        ...
        if t['profit_pct'] > 0: wins += 1
    
    wr = (wins / len(trade_history) * 100) if trade_history else 0
    print(f"\nWin Rate: {wr:.1f}% ({wins}/{len(trade_history)})")
    print("="*80)
