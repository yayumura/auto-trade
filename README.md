...
| :--- | :--- | :--- |
| **`python backtest.py`** | 過去の価格データを用いた戦略の**真実性の検証**。 | 通算利益、**月次勝率**、売買回数、シャープレシオが出力されます。 |
| **`python optimizer.py`** | 今の相場に最も適合する**パラメータの探索**。 | 全30通り以上の組み合わせを検証し、**安定性が高い順のランキング表**が出力されます。 |
...
- **安心設計**: `backtest.py` は読み取り専用の検証用であり、実行しても実際の口座への発注は一切行われません。
