...
1.  **プロフェショナル・複数期間バックテスター (`backtest.py`)** [UPGRADED]
    ...
    - 各期間の損益、勝率を一覧表で出力し、戦略の持続性と相場適応力を即座に可視化。
    - ルックアヘッド・バイアスを排除し、株式分割や配当も自動調整（auto_adjust）された高精度なシミュレーション。
2.  **ハイブリッド・リアルタイム監視エンジン**
    - 朝の「モーニング・スクリーニング」で全500銘柄以上から 50 銘柄を精査。
    - 日中は kabuステーション API の板情報（PUSH配信）と同期し、30秒単位でポジションを自動制御。
3.  **マルチ・レジーム適応ロジック (Strategy 4.2)** [LATEST - 🚀 66% Win Rate]
    - 相場を **BULL（強気）/ RANGE（揉み合い）/ BEAR（弱気）** に自動分類し、各環境に最適化したエントリーを実行。
    - **BULL**: 強い銘柄の押し目を狙う **"SMA20 Pullback + SMA5 Momentum Check"** 戦略。
    - **RANGE**: より厳格な基準（RSI < 30）で反転を狙う **"RSI Mean Reversion"** 戦略。
    - **Professional Exit Management**: ATRベースの多段階利確（3.0x ATR）と追従トレール（2.0x ATR）により、利益最大化を実現。
...
### 2. バックテストで戦略を検証 [ENHANCED]
本システムには、プロフェッショナルな二段階検証プロセスが搭載されています。

- **Phase 1: 高速・基本検証（デフォルト）**
  主要34銘柄（トヨタ、ソニー、三菱UFJ等）を使用して、戦略の基本性能を迅速に確認します。
  ```bash
  python backtest.py
  ```

- **Phase 2: 厳格・ストレステスト（Phase 2）**
  TOPIX 100銘柄相当（100銘柄）の広範な銘柄セットで、戦略の普遍性と頑健性を検証します。
  ```bash
  python backtest.py --all
  ```
※ どちらのモードでも、5日ごとの複数期間サマリー（相場適応力の確認）と、直近20日間の詳細レポートが自動で出力されます。
