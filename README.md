...
3.  **マルチ・レジーム適応ロジック (Strategy 3.0)** [LATEST]
    - 相場を **BULL（強気）/ RANGE（揉み合い）/ BEAR（弱気）** に自動分類し、各環境に最適化したエントリーを実行。
    - **BULL**: 強い銘柄の押し目を狙う **"SMA20 Pullback"** 戦略。
    - **RANGE**: 売られすぎからの反発を狙う **"RSI Mean Reversion"** 戦略。
    - **Symmetrical Time-Stop**: 3日間の期限付き決済により、横ばい相場での資金ロックを回避し、微益を確実に積み上げる設計。
